...
public class SimpleExpressionParser implements ExpressionParser {
	...
	/**
	 * This is the secound parse function (A) with grammer A := A+M || M.
	 * @param str String to be parsed
	 * @param parent Expression lastParsed
	 * @returns an Expression based on the grammer.
	 */
	...
	private Expression ParseA(String str, Expression parent) {
		...
		if (str.contains("+") && !(str.startsWith("(") && str.endsWith(")"))) {
			...
			if(expression != null) {
				...
				((AdditionExpression) expression).addSubexpression(ParseA(str.substring(0, crnt), expression));
				((AdditionExpression) expression).addSubexpression(ParseM(str.substring(crnt + 1), expression));
				return expression;
			}
		}
